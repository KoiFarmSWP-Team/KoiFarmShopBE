...
public class VarietyAPI {
    ...
    @PostMapping(value = "/api/variety/{id}")
    public ResponseEntity updateVariety(@PathVariable long id, @RequestBody Variety variety)
    {
        varietyService.updateVariety(id, variety);
        return ResponseEntity.ok().build();
    }
}
